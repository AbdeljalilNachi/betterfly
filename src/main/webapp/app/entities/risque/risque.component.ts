...
export class RisqueComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.risqueService
        ...
        .subscribe(
          (res: HttpResponse<IRisque[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInRisques(): void {
    this.eventSubscriber = this.eventManager.subscribe('risqueListModification', () => this.loadPage());
  }
