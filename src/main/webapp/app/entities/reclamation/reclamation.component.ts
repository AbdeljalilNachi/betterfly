...
export class ReclamationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.reclamationService
        ...
        .subscribe(
          (res: HttpResponse<IReclamation[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInReclamations(): void {
    this.eventSubscriber = this.eventManager.subscribe('reclamationListModification', () => this.loadPage());
  }
